...
class DatabaseOutputWriter(OutputWriter):
    ...
    def __init__(self, hashtag, session_id):
    ...
    def create(cls, mr_spec, shard_number, shard_attempt, _writer_state=None):
        ...
        return cls(hashtag, session_id)

    def write(self, data):
        ctx = context.get()
        db_pool = ctx.get_pool('database_pool')
        if not db_pool:
            db_pool = _DatabasePool(ctx=ctx, session_id=self._session_id, hashtag=self._hashtag)
    ...
    def to_json(self):
        return {self.HASHTAG: self._hashtag, self.SESSION_ID: self._session_id}
...
class _DatabasePool(context.Pool):
    ...
    def __init__(self, ctx=None, session_id=None, hashtag=None):
        self._actions = []
        self._size = 0
        self._ctx = ctx
        self._session_id = session_id
    ...
    def flush(self):
        if self._actions:
            for output in self._actions:
                RequestManager.addTweetsPerHour(self._session_id, self._hashtag, output)
        self._actions = []
        self._size = 0
